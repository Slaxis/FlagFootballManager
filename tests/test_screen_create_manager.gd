...
func test_a_perk_chip_can_be_taken_and_dropped(t: TestHelper) -> void:
	...
	# BY CODE, which is what the chip says now — the name moved to the tooltip
	# when the talents joined INT/PER/CAR in three letters.
	#
	# The roll may already have spent the balance on a talent of its own — the
	# ex-player lives three to six years, and a career hands out talents. Give
...
func test_every_talent_is_a_word_with_its_name_on_the_tooltip(t: TestHelper) -> void:
	...
	if screen == null or font == null or perks == null:
		t.fail("não consegui instanciar a tela"); return
	...
	for node: Variant in _collect(screen, "Button", []):
		...
		for id: String in perks.perk_ids():
			...
			t.check(chip.tooltip_text.contains(perks.label(id)),
				"o chip '%s' não diz em lugar nenhum que é '%s'" % [
					chip.text, perks.label(id)])
	...
	t.equal(found.size(), perks.perk_ids().size(), "talentos desenhados na tela")
	_close(screen)
