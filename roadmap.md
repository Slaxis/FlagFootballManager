...
| **B.9** | `vaga-e-slot` | **A vaga vazia virou porta.** Clicar numa vaga da escalação ou numa cadeira da comissão abre a lista de quem pode ocupá-la, ordenada pela afinidade NAQUELA posição — o caminho inverso de ordenar a tabela e clicar na célula, e os dois valem. `_affinity()` é uma função só, lida pela célula e pela lista. A lista é o elenco inteiro (decisão 47) e quem já está sentado diz onde. `_change_seat()` é funil único, que é onde o `C.1` vai cobrar. A presidência não abre | ✅ |
