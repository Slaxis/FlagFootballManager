...
# Serializes The.session + module-specific extras via callbacks.
extends Node
...
signal save_requested()  # emit to collect extras from active scenes
signal load_completed()  # emit after state restored

# Module scenes can listen to `save_requested` to inject extra data
# via `SaveManager.add_extras(key, data)` before the write completes.
...
func save_game() -> bool:
	...
	var payload: Dictionary = {
		...
		"session": The.session.duplicate(true),
		"lang": I18n.lang,
...
func apply_save(payload: Dictionary) -> void:
	...
	if session is Dictionary:
		The.session = (session as Dictionary).duplicate(true)
...
# --- Settings (persists across careers and installs) ---
