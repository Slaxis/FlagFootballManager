...
const ENGINE_CONFIG := "res://engine/engine.json"
...
func set_module(id: String) -> bool:
	...
	if mm == null:
		...
		return false
	var content_roots: Array[String] = mm.get_content_roots(key)
...
func list_json_by_group(group_id: String) -> Array[Dictionary]:
	...
	for path: String in paths:
		...
		if raw.is_empty():
			continue
		var raw_group: String = String(raw.get("group", "")).strip_edges().to_lower()
		if raw_group != group:
			continue
		results.append(raw)
	return results
